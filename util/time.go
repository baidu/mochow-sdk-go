...
func FormatRFC822Date(timestampSecond int64) string {
	tm := time.Unix(timestampSecond, 0).UTC()
	return tm.Format(RFC822Format)
}

func ParseRFC822Date(timeString string) (time.Time, error) {
	return time.Parse(RFC822Format, timeString)
}
...
func ParseISO8601Date(timeString string) (time.Time, error) {
	return time.Parse(ISO8601Format, timeString)
}
